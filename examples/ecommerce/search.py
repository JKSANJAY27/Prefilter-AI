...
search.py  —  hybrid search pipeline:
    ...
    natural language query
        │
        ▼
    prefilter-ai (structured JSON extraction)    ← hard filters
        │
        ▼
...
def _parse_operator(value: str | float | int) -> dict | None:
    """
    Turn a prefilter-ai operator string into a ChromaDB where-clause fragment.
    e.g. "lt:200"  →  {"$lt": 200.0}
...
# Map from prefilter-ai field names → ChromaDB metadata field names
FIELD_MAP = {
    "brand": "brand",
    "product": "product",
    "feature": "feature",
...
def build_chroma_where(parsed: dict) -> dict | None:
    """
    Convert a prefilter-ai JSON dict into a ChromaDB $and where-clause.
...
def hybrid_search(
    ...
    Run the full hybrid search pipeline:
      1. Parse query with prefilter-ai → structured JSON
