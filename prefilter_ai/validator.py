...
class ConflictDetector:
    ...
    def _check_pricing_feasibility(self, ir: IntermediateRepresentation) -> list[ConflictTag]:
        ...
        if domain == "real_estate":
            _HIGH_COST_CITIES = {"san francisco", "new york", "nyc", "london", "singapore", "hong kong", "zurich"}
        ...
        if domain == "jobs":
            if salary_min > 250_000:
                tags.append(ConflictTag(
                    message=f"Feasibility conflict: Very few roles offer $250k+ base salary. You may see very few results.",
