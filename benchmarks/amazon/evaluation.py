...
Evaluates THREE search pipelines on constraint satisfaction ONLY:
    ...
    [B] prefilter-ai hybrid     — structured filter → vector search
...
─────
    python benchmarks/amazon/evaluation.py \
        --hf-dataset JKSANJAY27/amazon-search-dataset
...
try:
    ...
    from prefilter_ai import PrefilterAI as SearchExpert

    PIPELINE_AVAILABLE = True
except ImportError:
    PIPELINE_AVAILABLE = False
    print("⚠  prefilter-ai or chromadb not installed.")
...
def build_chroma_where(parsed: dict) -> dict | None:
    """
    Build a ChromaDB where-clause from prefilter-ai parsed output.
    ...
    Specifically excluded from filtering:
      - product / brand / feature: prefilter-ai phrases these as natural
...
if __name__ == "__main__":
    ...
    if not PIPELINE_AVAILABLE:
        print("❌  Install prefilter-ai and chromadb.")
        sys.exit(1)
