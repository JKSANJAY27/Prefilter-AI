...
Three evaluation modes run on the same held-out test samples:
  ...
  1. STRUCTURED (prefilter-ai SLM adapters)
...
DATASET_REPO = "JKSANJAY27/prefilter-ai"
...
MODEL_NAMES = {
    "json_output": "prefilter-ai-json-0.8b",
    "yaml_output": "prefilter-ai-yaml-0.8b",
    "toml_output": "prefilter-ai-toml-0.8b",
    "csv_output": "prefilter-ai-csv-0.8b",
    "xml_output": "prefilter-ai-xml-0.8b",
}
...
def evaluate_spacy_baseline(
    ...
    try:
        ...
        raise ImportError(
            "prefilter_ai.spacy_extractor not found. "
            "Make sure spacy_extractor.py is in the prefilter_ai package."
        )
...
def print_leaderboard(
    ...
    print(
        "║          SLM (prefilter-ai) vs. spaCy NER vs. PURE VECTOR SEARCH          ║"
    )
    print(
        "╠════════════════════════════════════════════════════════════════════════════╣"
    )
    print(
        f"║  {'Metric':<22} {'prefilter-ai':>14} {'spaCy NER':>12} {'Vector Search':>14} {'Δ(SLM-spaCy)':>13}  ║"
    )
    print(
        "╠════════════════════════════════════════════════════════════════════════════╣"
    )
    ...
    if abs(gap_spacy) < 0.02:
        ...
        print(
            f"\n📊  prefilter-ai outperforms spaCy NER on Key F1 by "
    ...
    if gap_vec > 0.05:
        print(
            f"\n📊  prefilter-ai outperforms pure vector search on Key F1 by "
